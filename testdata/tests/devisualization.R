...
detests<-dir("DEtestresults",full.names = TRUE,no..=TRUE)
detestsnames<-dir("DEtestresults",full.names = FALSE,no..=TRUE)
ntests<-length(detests)
...
didx<-which(detestsnames %in% d)
defaultnames<-detestsnames[didx]
defaults<-detests[didx]
tidx<-which(!(detestsnames %in% d))
detests<-detests[tidx]
detestsnames<-detestsnames[tidx]
#sim data
...
if(!dir.exists("visualizedderesults")){
  dir.create("visualizedderesults")
}
...
while(i<=(ntests-2)){
  # while(i<=2){
  setwd("C://UCBERKELEY//ASlab//rna-seq//testdata")
  testi<-c(detests[i],detests[i+1])
  testnamei<-c(detestsnames[i],detestsnames[i+1])
  ...
  for(j in 1:length(predlist)){
    ...
    #get filtered genes
    predjk<-predj[3]
    predjk<-readRDS(predjk)
    genek<-rownames(predjk$counts)
    for(k in 1:length(methodnames)){
      ...
      #deseq2
      if(methodnamek=="defdeseq2.rds"|methodnamek=="tccdeseq2.rds"){
        for(c in 1:length(contrastlist)){
          pred_pval[,c]<-results(predjk,contrast = contrastlist[[c]])[,"padj"]
          pred_pval[is.na(pred_pval[,c]),]<-1 #set outliers as nde genes
        }
        DEdone<-TRUE
      }
      #lmvoom
      if(methodnamek=="deflmvoom.rds"|methodnamek=="tcclmvoom.rds"){
        pred_pval[,1:3]<-predjk$p.value[,2:4]
        DEdone<-TRUE
      }
      #bayseq
      if(methodnamek=="defbayseq.rds"|methodnamek=="tccbayseq.rds"){
        groupk<-names(predjk@groups)[2:length(predjk@groups)]
        pcounts<-c(0,0,0)
        pred_pval[,]<-1
        for(gp in 1:length(groupk)){
          startpt<-as.integer(substr(groupk[[gp]],3,3))
          tc<-topCounts(predjk,group = groupk[[gp]],number = nrow(predjk@data))
          repidx<-rownames(pred_pval[rownames(tc),startpt:3]>tc[,paste0("FWER.",groupk[[gp]])])
          pred_pval[repidx,startpt:3]<-tc[repidx,paste0("FWER.",groupk[[gp]])]
        }
        DEdone<-TRUE
      }
  ...
  #plot roc curve
  setwd("C://UCBERKELEY//ASlab//rna-seq//testdata//visualizedderesults")
  if(!dir.exists("roc")){
    dir.create("roc")
  }
  setwd("roc")
