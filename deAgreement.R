...
for(i in 1:length(sample_id)){
  ...
  for(j in 1:length(de_method)){
    if(de_method[[j]]=="voom"){
      ...
      vfit<-lmFit(v,design)
      vfit<-eBayes(vfit)
      degs<-topTable(vfit,number = Inf,p.value = 1,coef='conditionDay.5')
...
for(i in 1:length(de_res)){
  ...
  commonGene<-rownames(dei[[1]])
  for(k in 2:length(dei)){
    commonGene<-commonGene[which(commonGene %in% rownames(dei[[k]]))]
  }
  for(lfc_thresh in comparison_lfc){
    for(c in 1:length(comparison)){
      vennlist_input<-vector("list",length = length(dei))
